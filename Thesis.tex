...
\newglossaryentry{LIDAR}{name={LIDAR},description={Short for ``Light Detection and Ranging''. It is a type of sensor, that is using light in order to measure the distance to obstacles in the environment. It can be used to create 3D representations of areas}}
...
\newglossaryentry{KidnappedRobotProblem}{name={kidnapped robot problem},description={Describes the problem when a robot is taken and brought to a different location while a localization is running. The problem is that the robot is now localized in the wrong position and thus the localization fails. Kidnapped robot problem is often used interchangeably with localization failure}}
...
\newglossaryentry{Kernel}{name={kernel},description={When used with Gaussian processes kernels are also referred to as covariance function. They are used to measure the difference between two inputs. They can lift the inputs into a higher space, while the computations still happen in the input space. Thus one can save computation time by applying using kernels. They can be used whenever an operation can be represented by a dot product}}
